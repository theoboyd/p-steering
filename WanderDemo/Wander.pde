...
class Wander extends Steering {
  ...
  PVector calculateRawForce() {
    ...
    if (PVector.dist(target, agent.position) > radius) {
      ...
      /*
      PVector targetLocal = PVector.add(wanderTarget, new PVector(agent.wanderDistance, 0));
      PVector targetWorld = agent.toGlobalSpace(targetLocal);
      return PVector.sub(targetWorld, agent.position);
      */
      
      // Choose a centre-to-point vector c
      //PVector c = new PVector(agent.position.x, agent.position.y);
